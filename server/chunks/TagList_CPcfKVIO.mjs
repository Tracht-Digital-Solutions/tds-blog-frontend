...
var $$TagList = createComponent(($$result, $$props, $$slots) => {
	const Astro = $$result.createAstro($$props, $$slots);
	Astro.self = $$TagList;
	const { posts, tags, lang = "de", activeTag = null, label = null, class: className } = Astro.props;
	...
	return renderTemplate`${tagList.length > 0 && renderTemplate`${maybeRenderHead($$result)}<div${addAttribute(className, "class")}>${label && renderTemplate`<p class="eyebrow mb-3">${label}</p>`}<div class="flex flex-wrap gap-2">${tagList.map((t) => renderTemplate`${renderComponent($$result, "TagChip", $$TagChip, {
		"tag": t,
		"lang": lang,
		"active": t === activeTag
	})}`)}</div></div>`}`;
