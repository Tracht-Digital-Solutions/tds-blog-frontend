import { A as renderTemplate, N as addAttribute, V as createAstro, j as maybeRenderHead } from "./sequence_rQseZV6z.mjs";
...
var $$RssInfo = createComponent(($$result, $$props, $$slots) => {
	...
	return renderTemplate`${maybeRenderHead($$result)}<section class="jnl-tone-navy"><div class="tds-shell py-14 md:py-20"><p class="eyebrow" style="color: var(--color-accent-pink);">${t.eyebrow}</p><h1 class="display page-title mt-3">${t.heading[0]}<span class="accent-italic">${t.heading[1]}</span>${t.heading[2]}</h1><p class="mt-4 max-w-prose" style="color: rgba(255,255,255,.75); line-height: 1.6;">${t.lede}</p></div></section><section class="jnl-band jnl-tone-sand" aria-labelledby="rss-what"><div class="tds-shell"><h2 id="rss-what" class="display text-2xl md:text-3xl mb-3">${t.whatHead}</h2><p class="marginalia max-w-prose">${t.whatBody}</p></div></section><section class="jnl-band jnl-tone-tint" aria-labelledby="rss-feed"><div class="tds-shell"><h2 id="rss-feed" class="display text-2xl md:text-3xl mb-3">${t.feedHead}</h2><p class="marginalia max-w-prose mb-4">${t.feedHint}</p><div class="flex flex-wrap items-stretch gap-px" style="max-width: 36rem;"><code id="rss-feed-url" class="flex-1 min-w-0 px-4 py-3 text-sm truncate" style="font-family: var(--font-mono); background: var(--jnl-tile); color: var(--color-ink);">${FEED_URL}</code><button id="rss-copy" type="button"${addAttribute(FEED_URL, "data-copy")}${addAttribute(t.copy, "data-label-copy")}${addAttribute(t.copied, "data-label-copied")} class="px-4 py-3 text-sm font-semibold shrink-0" style="background: var(--color-surface-navy); color: #fff; cursor: pointer;">${t.copy}</button></div></div></section><section class="jnl-band jnl-tone-sand" aria-labelledby="rss-programs"><div class="tds-shell"><h2 id="rss-programs" class="display text-2xl md:text-3xl mb-2">${t.programsHead}</h2><p class="marginalia max-w-prose mb-6">${t.programsHint}</p><ul class="list-none p-0 m-0 tds-grid-auto tds-grid-roomy jnl-mosaic">${programs.map((p) => renderTemplate`<li class="list-none"><a${addAttribute(p.href, "href")} target="_blank" rel="noopener noreferrer" class="topic-card"><h3 class="topic-title">${p.name}</h3><p class="topic-desc">${p.desc}</p></a></li>`)}</ul><p class="pt-6"><a${addAttribute(FEEDLY_URL, "href")} target="_blank" rel="noopener noreferrer" class="inline-flex items-center gap-2 px-5 py-3 text-sm font-semibold no-underline" style="background: var(--color-accent); color: #fff;">${t.feedlyCta} →</a></p></div></section><section class="jnl-band jnl-tone-tint" aria-labelledby="rss-raw"><div class="tds-shell"><h2 id="rss-raw" class="display text-xl md:text-2xl mb-2">${t.rawHead}</h2><p class="marginalia max-w-prose">${t.rawBody}${" "}<a${addAttribute(FEED_PATH, "href")} class="link-underline text-[var(--color-accent)]">${FEED_PATH}</a>.</p></div></section><script>
  (function () {
