...
var m = {
	...
	nextBulletRegex: I((u) => new RegExp(`^ {0,${u}}(?:[*+-]|\\d{1,9}[.)])((?:[ 	][^\\n]*)?(?:\\n|$))`)),
	hrRegex: I((u) => new RegExp(`^ {0,${u}}((?:- *){3,}|(?:_ *){3,}|(?:\\* *){3,})(?:\\n+|$)`)),
	fencesBeginRegex: I((u) => new RegExp(`^ {0,${u}}(?:\`\`\`|~~~)`)),
	headingBeginRegex: I((u) => new RegExp(`^ {0,${u}}#`)),
	htmlBeginRegex: I((u) => new RegExp(`^ {0,${u}}<(?:[a-z].*>|!--)`, "i")),
	blockquoteBeginRegex: I((u) => new RegExp(`^ {0,${u}}>`))
};
var Oe = /^(?:[ \t]*(?:\n|$))+/;
var we = /^((?: {4}| {0,3}\t)[^\n]+(?:\n(?:[ \t]*(?:\n|$))*)?)+/;
var ye = /^ {0,3}(`{3,}(?=[^`\n]*(?:\n|$))|~{3,})([^\n]*)(?:\n|$)(?:|([\s\S]*?)(?:\n|$))(?: {0,3}\1[~`]* *(?=\n|$)|$)/;
...
var le = (u) => k(K).replace("hr", q).replace("heading", " {0,3}#{1,6}(?:\\s|$)").replace("|lheading", "").replace("|table", "").replace("blockquote", " {0,3}>").replace("fences", " {0,3}(?:`{3,}(?=[^`\\n]*(?:\\n|$))|~~~)[^\\n]*(?:\\n|$)").replace("list", u).replace("html", "</?(?:tag)(?: +|\\n|/?>)|<(?:script|pre|style|textarea|!--)").replace("tag", Q).getRegex();
var Ee = le(/ {0,3}(?:[*+-]|1[.)])[ \t]+[^ \t\n]/);
var Me = le(/ {0,3}(?:[*+-]|\d{1,9}[.)])(?:[ \t]|\n|$)/);
var J = {
...
var G = k(/(?:\[(?:brackets|\\[\s\S]|[^\[\]\\])*\]|\\[\s\S]|`+(?!`)[^`]*?`+(?!`)|``+(?=\])|[^\[\]\\`])*?/).replace("brackets", /\[(?:\\[\s\S]|[^\[\]\\])*\]/).getRegex();
...
var F = {
	...
	delLDelim: tt,
	delRDelim: rt,
	url: k(/^((?:protocol):\/\/|www\.)(?:[a-zA-Z0-9\-]+\.?)+[^\s<]*|^email/).replace("protocol", ie).replace("email", /[A-Za-z0-9._+-]+(@)[a-zA-Z0-9-_]+(?:\.[a-zA-Z0-9-_]*[a-zA-Z0-9])+(?![\w-])/).getRegex(),
...
function ee(u, e) {
	let n = u.replace(m.findPipe, (r, o, i) => {
		let l = !1, a = o;
		for (; --a >= 0 && i[a] === "\\";) l = !l;
		return l ? "|" : " |";
...
`), t = e.length - 1;
	...
	return e.length - t <= 2 ? u : e.slice(0, t + 1).join(`
`);
}
function fe(u, e) {
...
function xe(u, e, t, n, s) {
	let r = e.href, o = e.title || null, i = u[1].replace(s.other.outputLinkReplace, "$1"), l = u[0].charAt(0) === "!";
	n.state.inLink = !0;
	let a = n.state.linkEmitted, p = n.state.inRawBlock;
	n.state.linkEmitted = !1;
	let c = n.inlineTokens(i), h = n.state.linkEmitted;
	if (n.state.linkEmitted = a, n.state.inLink = !1, !l) {
		if (h) {
...
`), s = "", r = "", o = [];
			for (; n.length > 0;) {
				let i = !1, l = [], a = 0;
				for (; a < n.length; a++) if (this.rules.other.blockquoteStart.test(n[a])) l.push(n[a]), i = !0;
...
`),
		...
		if (n.length === s.length) {
			...
			for (let i of r) o.rows.push(ee(i, o.header.length).map((l, a) => ({
				text: l,
...
`;
	...
	link({ href: e, title: t, text: n, tokens: s, autolink: r }) {
		let o = r ? R(n, !0) : this.parser.parseInline(s), i = Y(e);
		if (i === null) return o;
		e = R(i, r);
		let l = "<a href=\"" + e + "\"";
...
f.options = f.setOptions = function(u) {
	return M.setOptions(u), f.defaults = M.defaults, j(f.defaults), f;
};
f.getDefaults = A;
f.defaults = T;
function gt(...u) {
	return M.use(...u), f.defaults = M.defaults, j(f.defaults), f;
}
f.use = gt;
