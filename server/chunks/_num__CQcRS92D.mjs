...
import { A as renderTemplate, N as addAttribute, V as createAstro, j as maybeRenderHead, w as renderComponent } from "./sequence_DMm2rtV7.mjs";
import { t as createComponent } from "./compiler_DOy9cg7b.mjs";
import { t as $$Layout } from "./Layout_BiZ0WZa5.mjs";
import { d as paginate, i as archivePage } from "./cache_COQztzBT.mjs";
...
import { t as archiveAlternate } from "./alternates_DPvBZahy.mjs";
import { t as $$BlogPostCard } from "./BlogPostCard_B-ujZw6n.mjs";
import { t as translations } from "./i18n_hxU9TLLi.mjs";
...
var $$Num = createComponent(async ($$result, $$props, $$slots) => {
	...
	}, { "default": ($$result) => renderTemplate`${maybeRenderHead($$result)}<main id="main" class="tds-shell py-16 lg:py-24"><header class="mb-10"><p class="section-num mb-4">${t.blog.label} · Page ${page} / ${pageCount}</p><h1 class="display page-title">${t.blog.headline}${" "}<span class="accent-italic">${t.blog.headlineAccent}</span></h1><p class="marginalia mt-3 max-w-prose">Older notes from the archive. Equally valid, just further back in time.</p></header><ul class="list-none p-0 m-0">${items.map((p) => renderTemplate`${renderComponent($$result, "BlogPostCard", $$BlogPostCard, {
		"slug": p.slug,
		"category": p.category,
		"title": p.title,
		"excerpt": p.excerpt,
