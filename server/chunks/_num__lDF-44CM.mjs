...
import { A as renderTemplate, N as addAttribute, V as createAstro, j as maybeRenderHead, w as renderComponent } from "./sequence_rQseZV6z.mjs";
import { t as createComponent } from "./compiler_CHD7gGAf.mjs";
import { c as translations, t as $$Layout } from "./Layout_BrLUihmw.mjs";
import { d as paginate, i as archivePage } from "./cache_TutC0xtL.mjs";
...
import { t as archiveAlternate } from "./alternates_Bde-7Osz.mjs";
import { t as $$BlogPostCard } from "./BlogPostCard_CA1ApAyL.mjs";
...
var $$Num = createComponent(async ($$result, $$props, $$slots) => {
	...
	}, { "default": ($$result) => renderTemplate`${maybeRenderHead($$result)}<main id="main"><header class="jnl-band jnl-tone-navy"><div class="tds-shell"><p class="section-num mb-4">${t.blog.label} · Page ${page} / ${pageCount}</p><h1 class="display page-title">${t.blog.headline}${" "}<span class="accent-italic">${t.blog.headlineAccent}</span></h1><p class="marginalia mt-3 max-w-prose">Older notes from the archive. Equally valid, just further back in time.</p></div></header><section class="jnl-band jnl-tone-tint"><div class="tds-shell"><ul class="jnl-stack">${items.map((p) => renderTemplate`${renderComponent($$result, "BlogPostCard", $$BlogPostCard, {
		"slug": p.slug,
		"category": p.category,
		"title": p.title,
		"excerpt": p.excerpt,
