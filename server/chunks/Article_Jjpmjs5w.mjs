import { A as renderTemplate, At as boolean, Dt as ZodDate, Ft as string, It as union, Lt as _coercedDate, Mt as literal, N as addAttribute, Nt as number, Ot as _enum, P as defineScriptVars, Pt as object, T as Fragment, V as createAstro, j as maybeRenderHead, jt as discriminatedUnion, kt as array, w as renderComponent, z as unescapeHTML } from "./sequence_BU0hsnhb.mjs";
import { t as createComponent } from "./compiler_CDBSO3Ry.mjs";
import { i as categoryHref, o as ProductCard, r as authorHref, s as renderScript, t as $$Layout } from "./Layout_Cs4gRr25.mjs";
import { b as assertKeyAccepted, f as adsConfig, h as effectiveAdsMode, l as categorySlug, p as blogSnippets, r as contentCache, x as siteKeyHeaders } from "./cache_C5L6PGMb.mjs";
import { i as contentApiBase } from "./connection_CNpSsWkT.mjs";
import { n as siteConfig, t as pageTitle } from "./seo_C65aaSyf.mjs";
import { i as slugify$1, n as renderBlockHtml } from "./localizedPost_Dcjxwj7o.mjs";
import { n as PostCard } from "./PostCard_f9aw5YH9.mjs";
...
var $$RelatedArticles = createComponent(($$result, $$props, $$slots) => {
	...
	const { posts, currentSlug, currentCategory, lang, limit = 3, class: className } = Astro.props;
	...
	return renderTemplate`${picks.length > 0 && renderTemplate`${maybeRenderHead($$result)}<aside${addAttribute(["jnl-panel jnl-tone-tint", className], "class:list")} aria-labelledby="related-heading"><p class="section-num mb-3">${labels.eyebrow}</p><h2 id="related-heading" class="display-tight text-3xl mb-6">${lang === "de" ? "Ähnliche Artikel." : "Adjacent articles."}</h2><ul class="tds-grid-auto jnl-mosaic list-none p-0 m-0">${picks.map((p) => renderTemplate`<li class="post-card-slot">${renderComponent($$result, "PostCard", PostCard, {
		"post": p,
		"lang": lang
	})}</li>`)}</ul></aside>`}`;
...
var $$ProductSlot = createComponent(async ($$result, $$props, $$slots) => {
	...
	const { slotKey, lang = "de", category, layout = "strip", class: className } = Astro.props;
...
var $$Article = createComponent(async ($$result, $$props, $$slots) => {
	...
	return renderTemplate`${renderComponent($$result, "Layout", $$Layout, {
		...
		"lang": lang
	})}${renderComponent($$result2, "TagList", $$TagList, {
		"class": "focus-hide jnl-panel jnl-tone-sand",
		"tags": post.tags,
		"lang": lang,
		"label": "Tags"
	})}${(newerPost || olderPost) && renderTemplate`<nav class="focus-hide jnl-panel jnl-tone-tint grid sm:grid-cols-2 gap-px"${addAttribute(tx.nav, "aria-label")}>${newerPost ? renderTemplate`<a${addAttribute(postHref(newerPost.slug), "href")} class="jnl-tile block px-5 py-4 group"><p class="eyebrow mb-2">← ${tx.newer}</p><p class="display-tight text-lg group-hover:text-[var(--color-primary)] transition-colors">${newerPost.title}</p></a>` : renderTemplate`<span class="hidden sm:block" aria-hidden="true"></span>`}${olderPost && renderTemplate`<a${addAttribute(postHref(olderPost.slug), "href")} class="jnl-tile block px-5 py-4 group sm:text-right"><p class="eyebrow mb-2">${tx.older} →</p><p class="display-tight text-lg group-hover:text-[var(--color-primary)] transition-colors">${olderPost.title}</p></a>`}</nav>`}<footer class="jnl-panel jnl-tone-sand flex flex-wrap items-baseline justify-between gap-4 text-sm"><a${addAttribute(`${pfx}/`, "href")} class="link-underline text-[var(--color-accent)]">← ${tx.allArticles}</a><a${addAttribute(`${pfx}/rss`, "href")} class="link-underline text-[var(--color-muted)]">${tx.rss}</a></footer></div></article></main>${renderScript($$result2, "/home/runner/work/tds-blog-frontend/tds-blog-frontend/src/components/Article.astro?astro&type=script&index=0&lang.ts")}${renderScript($$result2, "/home/runner/work/tds-blog-frontend/tds-blog-frontend/src/components/Article.astro?astro&type=script&index=1&lang.ts")}${renderScript($$result2, "/home/runner/work/tds-blog-frontend/tds-blog-frontend/src/components/Article.astro?astro&type=script&index=2&lang.ts")}<script>(function(){${defineScriptVars({ interestTopics })}
    (function () {
      try {
