import { A as renderTemplate, At as number, D as renderSlot, Dt as _enum, F as createRenderInstruction, M as renderHead, Mt as string, N as addAttribute, Ot as array, P as defineScriptVars, T as Fragment$2, V as createAstro, j as maybeRenderHead, jt as object, kt as boolean, w as renderComponent, z as unescapeHTML } from "./sequence_rQseZV6z.mjs";
import { t as createComponent } from "./compiler_CHD7gGAf.mjs";
import { f as adsConfig, m as cookieBannerEnabled, u as getTaxonomy } from "./cache_TutC0xtL.mjs";
import { n as siteConfig } from "./seo_C65aaSyf.mjs";
import { i as isExcluded } from "./sitemapExclusions_C7XVdP4D.mjs";
...
var VIEW_TRANSITION_SCOPE = "tds-vt-list";
function transitionUpdate(update, types = ["list"]) {
	...
	transition.finished.finally(() => root.classList.remove(VIEW_TRANSITION_SCOPE));
}
var ease = [
	.2,
	.8,
...
var $$JournalHeader = createComponent(async ($$result, $$props, $$slots) => {
	...
	})}<div class="tds-sitebar__wide" data-astro-cid-eldh542x><a${addAttribute(contact, "href")} class="btn btn-primary no-underline" data-astro-cid-eldh542x>${cta}</a></div></div><div class="tds-sitebar__actions" data-astro-cid-eldh542x>${renderComponent($$result, "AccountMenu", AccountMenu, {
		"client:idle": true,
		"lang": lang,
		"data-astro-cid-eldh542x": true,
		"client:component-hydration": "idle",
...
function NewsletterSignup({ lang }) {
	...
	const submit = async (e) => {
		...
		try {
			...
			})).ok) transitionUpdate(() => flushSync(() => setState("done")));
			else setState("error");
		} catch {
			setState("error");
		}
	...
	return /* @__PURE__ */ jsx("section", {
		...
		children: /* @__PURE__ */ jsxs("div", {
			...
			children: [/* @__PURE__ */ jsxs("div", { children: [
				...
				className: "tds-vt-item",
				style: { "--tds-vt-name": "newsletter-signup" },
				children: [state === "done" ? /* @__PURE__ */ jsx("p", {
					role: "status",
					style: {
						fontSize: 16,
						lineHeight: 1.6,
...
var $$JournalFooter = createComponent(async ($$result, $$props, $$slots) => {
	...
	})}${maybeRenderHead($$result)}<footer class="blog-footer tds-tone-navy" style="padding: 48px 0 28px;"><div class="tds-shell"><div class="grid gap-9 sm:grid-cols-2 lg:[grid-template-columns:1.4fr_repeat(4,1fr)]"><div><p class="brand-wordmark text-lg mb-3 inline-flex items-center gap-2" style="color: #fff;"><span class="brand-logo brand-logo--inverse" aria-hidden="true"></span><span style="color: var(--color-accent-pink);">Digital</span></p><span aria-hidden="true" class="tds-brandbar tds-brandbar--sm tds-brandbar--on-dark mb-4"></span><p class="text-sm leading-relaxed" style="color: rgb(255 255 255 / 0.65); max-width: 32ch;">${blurb}</p><p class="mt-5 text-[0.8125rem]" style="font-family: var(--font-mono); color: rgb(255 255 255 / 0.6);">Schwarzenbek</p></div>${groups.map((g) => renderTemplate`<div><p class="eyebrow mb-4" style="color: rgb(255 255 255 / 0.6);">${g.head}</p><ul class="list-none p-0 m-0 flex flex-col gap-2.5">${g.items.map((it) => renderTemplate`<li><a${addAttribute(it.href, "href")} class="text-sm no-underline text-white/75 hover:text-white transition-colors">${it.label}</a></li>`)}</ul></div>`)}</div><div class="mt-9 pt-4 flex flex-wrap justify-between items-center gap-4 text-xs" style="border-top: 1px solid rgb(255 255 255 / 0.1); font-family: var(--font-mono); color: rgb(255 255 255 / 0.6);"><p>© ${year} Tracht Digital Solutions</p><div class="flex items-center gap-4">${renderComponent($$result, "ConsentLink", ConsentLink, {
		"client:idle": true,
		"lang": lang,
...
var $$Layout = createComponent(async ($$result, $$props, $$slots) => {
	...
	const consentCategories = !!ads && ads.enabled ? ["marketing"] : [];
	const showConsent = !bare && (consentCategories.length > 0 || await cookieBannerEnabled());
	return renderTemplate`<!-- data-surface selects the geometry layer from tds-shared's design
     ...
     .display, .display-tight and .eyebrow in the first place. --><html${addAttribute(lang, "lang")} data-surface="blog"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><!-- The cross-page transition switch, as early as it can be: Chrome decides the
         incoming page's opt-in at its first render, and the stylesheet linked at the
         end of this head often arrives later (tds-shared pageTransitionOptIn). --><style>${unescapeHTML(pageTransitionOptIn)}</style><meta name="description"${addAttribute(description, "content")}><meta name="theme-color" content="#050f68">${hidden && renderTemplate`<meta name="robots" content="noindex,nofollow">`}<link rel="icon" type="image/png" href="/favicon.png" sizes="any"><script>${unescapeHTML(themeBootstrapScript)}<\/script>${focusable && renderTemplate`<script>
      ...
      })();<\/script>`}${renderScript($$result, "/home/runner/work/tds-blog-frontend/tds-blog-frontend/src/layouts/Layout.astro?astro&type=script&index=1&lang.ts")}</body></html>`;
}, "/home/runner/work/tds-blog-frontend/tds-blog-frontend/src/layouts/Layout.astro", void 0);
//#endregion
export { PostCover as a, translations as c, categoryHref as i, emptyPlacement as l, TOOLS_URL as n, ProductCard as o, authorHref as r, transitionUpdate as s, $$Layout as t, renderScript as u };
